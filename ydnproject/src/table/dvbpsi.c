...
/* &dvbpsi_Demux, 4096, true,sizeof(dvbpsi_demux_t) */


void *dvbpsi_decoder_new( dvbpsi_callback_gather_t pf_gather,
			  const int i_section_max_size, const bool b_discontinuity, const int psi_size )
{
	...
	p_decoder->pf_gather		= pf_gather;            /* dvbpsi_Demux */
	p_decoder->p_current_section	= NULL;
	p_decoder->i_section_max_size	= i_section_max_size;   /* 4096 */
	p_decoder->b_discontinuity	= b_discontinuity;      /* true */
	p_decoder->i_continuity_counter = DVBPSI_INVALID_CC;    /* 0xff */
	p_decoder->p_current_section	= NULL;
