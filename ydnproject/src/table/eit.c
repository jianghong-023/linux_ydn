...
static dvbpsi_psi_section_t* NewEITSection(dvbpsi_eit_t* p_eit, int i_table_id,
                                           int i_section_number,bool b_private_indicator )
{
  ...
  //following modified by zjl 20160413
//  p_result->b_private_indicator = 1;
  p_result->b_private_indicator = b_private_indicator;
 //*******************************************************//
...
dvbpsi_psi_section_t* dvbpsi_eit_sections_generate(dvbpsi_t *p_dvbpsi, dvbpsi_eit_t *p_eit,
                                            uint8_t i_table_id,uint8_t i_section_number,uint8_t i_last_section_number,bool b_private_indicator)
{
  dvbpsi_psi_section_t *p_result = NewEITSection (p_eit, i_table_id, i_section_number , b_private_indicator);
  ...
  for (p_event = p_eit->p_first_event; p_event; p_event = p_event->p_next)
  {
         ...
         p_descriptor; p_descriptor = p_descriptor->p_next)
    {
      ...
      if (p_event_start - p_current->p_data + i_event_length > 4090)
      {
        ...
        p_current = NewEITSection (p_eit, i_table_id, ++i_last_section_number,0);
